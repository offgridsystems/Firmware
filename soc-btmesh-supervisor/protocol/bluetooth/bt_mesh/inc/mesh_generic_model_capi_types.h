...
#define MESH_DK_BLOCK_STATUS_SERVER_MODEL_ID 		0x1300
#define MESH_DK_BLOCK_STATUS_SETUP_SERVER_MODEL_ID 	0x1301
 ...
 * DK code addition
 *****************************************************/
struct mesh_DK_block_temperature_state {
   	// temperature sensors on the 3.7v clamp pcb
   	uint16_t temp_3_ntc1;
   	uint16_t temp_3_ntc2;
   	uint16_t temp_3_ntc3;
   	uint16_t temp_3_ntc4;
   	// temperature sensors on the 7.4v clamp pcb
   	uint16_t temp_7_ntc1;
    uint16_t temp_7_ntc2;
   	uint16_t temp_7_ntc3;
 	// temperature sensor on the block manager pcb
    uint16_t temp_ob_ntc1;
};

struct mesh_DK_block_voltage_state {
    // scaled voltage
    uint16_t voltage_3_scaled;
    uint16_t voltage_7_scaled;
};

struct mesh_DK_block_shunt_state {
   	uint8_t shunt_low;
   	uint8_t shunt_hi;
};

struct mesh_DK_block_fan_state {
    uint8_t fan;
};
...
typedef enum {
  ...
  mesh_DK_state_block_temperature 	= 0x40,
  mesh_DK_state_block_voltage 		= 0x41,
  mesh_DK_state_block_shunt			= 0x42,
  mesh_DK_state_block_fan			= 0x43,
...
struct mesh_generic_state {
  ...
  union {
     ...
     * DK addition
     *************************************************/
    struct mesh_DK_block_temperature_state block_temp;
	struct mesh_DK_block_voltage_state block_volt;
	struct mesh_DK_block_shunt_state block_shunt;
	struct mesh_DK_block_fan_state block_fan;
  };
};
...
typedef enum {
  ...
  mesh_DK_request_block_temperature = 0x40,
  mesh_DK_request_block_voltage 	= 0x41,
  mesh_DK_request_block_shunt		= 0x42,
  mesh_DK_request_block_fan			= 0x43,
...
struct mesh_generic_request {
  ...
  union {
    ...
    struct {
    	// scaled voltage readings
    	uint8_t voltage_3_scaled;
    	uint8_t voltage_7_scaled;
    } block_volt;
    struct {
    	uint8_t shunt_low;
    	uint8_t shunt_hi;
