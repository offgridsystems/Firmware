...
// 6/29/2016  TBE             Add 3-5VPMW for charge control on DAC2 output, Add Limp Mode output to A4-NC testpoint (PIN19)
// 4/17/2017  TBE     1717    Added fix for 2-5V PWM DAC for charge control - not updating during charge mode! 
//                            (Sketch -> Export compiled binary and then Show Sketch folder)
// 7/07/2016  TBE     2116    Added checksum for data comms to ensure good quality data, improved bogus bytes. Also
...
// 7/10/2017  TBE     2817    Keyword "Jul 12, 2017" Latched limp mode when SOC < 20%. Reset when Pack Supervisor power is cycled. 
//                            Also was no SOCv averaging so Fuel Gauge jumped around a lot. Averaged value so steadier .
// 7/24/2017  TBE     3017    SOCv sampled too early at startup and SOCv and TempV var not being initialized properly. 
//                            Insure no sampling of SOCv the first 10 seconds, and SOCv and TempV var now initialized to Vnominal 
//                            (This will allow the rider to get a few seconds of motoring after startup no matter the SOC condition. 
//                            We may want to max of 5 restarts in limp mode.
  ...
  */
  ...
  if (GoodComms) {
    if (Temp1 >= Temp2) {    // find highest/lowest Vcell - peak detector
      ...
      //if (Temp2 > Highest_Vcell) Highest_Vcell = ((Highest_Vcell + Highest_Vcell + Highest_Vcell + Temp2) / 4) ;// insure that one bad byte does not change the value
