...
// Compile for Teensy LC 24Mhz - smallest code
// Block Manager PCB Rev D production code for Teensy LC  -    10/26/2015  T Economu

// Date       Who     New Ver   WAS/IS changes
// 1Nov2015   TBE               Added software RTC for secs, mins, and hours, and DO NOT roll at with 4 billion in
...
//  3/15/2019 TBE               Removed snooze, libraray had a lot of problems.
//  3/27/2019 TBE     1319      Sync-ACK packet sent from PS to block, one at a time to speed comms
//  4/4.2019  TBE     1419      Very fast comms, make sure to turn on COP and Sleep modes for testing
//  4/14/2019 TBE     1619      Turn on COP watchdog timer. OOps was already on! Remove SPI interrupt - not used. OK oops again already removed!
//                              Turn (back) on SLEEP mode with wakeups for rising and lowering of cell voltages and comms wakeup from Supervisor using Duffs SNOOZE library 6.0
//   4/16/2019  TBE   1619      Named code = Working_block_with_deepsleep_1619 
//   6/26/2019  TBE   2619      Added CommTimer for 7 second block comm wait (only comm every 7 secs)     
//   7/1/20019  TBE   2719      Added one-block=per second timer so that either good comms or 1 second reset the timer (code in PS)
//   7/11/2019  TBE   2819      ****Breakthrough on comms throughput.**** Each block gets its own freq channel (equals its block address)
//   7/15/2019  TBE   2919      Optimized for all blocks communicating every 200ms or so, so about 3 sec for all 20 blocks
//  7/22/2019   TBE   2919-3    Measured currents at 1.47ma sleep mode, wakes every 60 seconds for about 4 secs and uses 30ma while 
//                              awake. So sleep uses about 2ma average over 24h/365 days is 17ah. Using 3.5ah cells, that would be 35 ah total
//                              or about 2 years in storage if stored at full charge.   
...
#include "Arduino.h"
#include <cstdint>  // used for type definitions
  

const uint16_t VERSION = 3019;   // 5215 = 52th week of 2015

/*     Comms library from Radio Head ver 1.8.9 at http://www.airspayce.com/mikem/arduino/RadioHead/examples.html
  nrf24_reliable_datagram_client.pde
  C:\Users\Tim Economu\Desktop\18650 pack project\Software\Arduino software\RF24 code\RadioHead
...
#define CLIENT_ADDRESS  16       // BLOCK number ALWAYS start from 1 (NOT 0) 255 MAX!!
#define SERVER_ADDRESS  60     // must always talk to this PACK SUPERVISOR addresss
...
RH_NRF24 driver(9, 10); // define CE and CSN pins for Teensy LC/ 3.1
// Class to manage message delivery and receipt, using the driver declared above
RHReliableDatagram manager(driver, CLIENT_ADDRESS);


// use Duffs latest library version Snooze v6.3.4 https://github.com/duff2013/Snooze
// Apr 2019 
#include <Snooze.h>   // used for deep sleep mode for IDD of around 150uA for (Teensy LC).
// must be global
// Load drivers for snooze block
SnoozeDigital digital;      // wake on digital input trip
SnoozeTimer timer;          // wake on timer timeout
...
// turn debugging feature on or off. debug = YES for factory test or debug, NO for Production
bool debugg = NO;
//bool debugg = YES;
bool debugsleep = NO;
//bool debugsleep = YES;
...
const byte LED2yellow = 7;
const byte LED2green = 8;
const byte LED1yellow = 2;
...
uint8_t CommTimer = 0 ;   // wait for n sec inteval to transmit packets to Pack Supervisor board
//const int PACKET_DELAY_SEC = 7; // n sec interval between packets - seems like a #10 and #12 are being dropped now and then
const int PACKET_DELAY_SEC = 2; // n sec interval between packets - seems a couple blocks get dropped now and then if this is higher on a 20 block system
...
********************************************************/
const int TIMEBETWEENSLEEP = 4;    // 4 sec between hibernate session if sleepmode, or 80 sec if chg/disch
int DpSleep_timer = TIMEBETWEENSLEEP*20;    // start up from boot with 80 seconds of trying to find supervisor
...
void setup() {    // ==============================================================
  ...
  timer.setTimer(64000); // 64 seconds of hibernate
  ...
  else Serial.println("init passed");

  // Defaults after init are 2.402 GHz (channel 2), 2Mbps, 0dBm
  // new 7/10/2019
  //driver.setChannel(1);
  //if (!driver.setChannel(108)) Serial.println("setChannel failed");
  //if (!driver.setChannel(CLIENT_ADDRESS)) Serial.println("setChannel failed");   // must be set in RF24 lib
  // new 7/10/2019
  //if (!driver.setRF(RH_NRF24::DataRate250kbps, RH_NRF24::TransmitPower0dBm)) Serial.println("setRF failed");
 // else  Serial.println("setRF passed");
  ...
  pinMode(LED1yellow, OUTPUT);      // enable digital output for turning on LED indicator
  pinMode(LED2green, OUTPUT);      // enable digital output for turning on LED indicator
  pinMode(LED2yellow, OUTPUT);      // enable digital output for turning on LED indicator
  ...
  digitalWrite(LED1yellow, GLOW);   delay(800);  // LED on for 1 second
  WatchdogReset();  // reset the watchdog timer
  ...
  digitalWrite(LED2yellow, GLOW);   delay(800);  // LED on for 1 second
  WatchdogReset();  // reset the watchdog timer
  ...
  digitalWrite(LED1yellow, DARK);
  digitalWrite(LED2green, DARK);
  digitalWrite(LED2yellow, DARK);
  Serial.println("4:Now test NTCs");
  WatchdogReset();  // reset the watchdog timer

  // default RF24 manager settings are 2000ms timeout and 3 retries. These are too long for COP at one second
  ...
  manager.setTimeout(20);       // timeout can go higher if needed, but always try to go lower...
  manager.setRetries(1);        // retries are HUGE time wasters
...
void WatchdogReset (void) {     // use COP registers on the Teensy LC to set for one second timeout
  SIM_SRVCOP = 0x55;
  SIM_SRVCOP = 0xAA;
}

//debug maybe take this out for production
...
void loop() {  // ================================================================
  ...
  if ((currentmicros - nextmicros) >= interval)  // if 1000000 microseconds have gone by...
  {
    ...
    // new 7/10/2019
    //if (CommTimer <= 255) CommTimer = CommTimer + 1;    // 
    CommTimer = CommTimer + 1;    // rolls over at 255 ---> 0
    // new 7/10/2019
...
throwaway_t:      ;
  ...
  if ((HiShunt == ON) || (LoShunt == ON)) {
    ...
    digitalWrite(LED2yellow, GLOW);    // light red LED if balancing
...
endoftest:
  ...
  // use debug here if working on BM boards that are on USB power only (no block connection) because they will hang in low voltage hibernate otherwise
  if (debugsleep == NO) {
    // if mode in sleep, turn all perpherals off and put micro to deep sleep
    /********************************************************
      feed the hybernate function its wakeup parameters. Then go to deepSleep.
    ********************************************************/
    if ((BlockMode == SLEEPING) && (DpSleep_timer == 0)) {
      DpSleep_timer = TIMEBETWEENSLEEP;    // set the time between hybernate events
      ...
      int What_INT = 0;     // debug remove for seeing what interrupt woke up the CPU

      // if sleepmode, and if either cell is low, hibernate until charge > 3.2V/cell, otherwise hibernate only for 64 seconds
      if ((Hist_Hiside_CellV < (Vcell_Low_Spec + 0.35)) ||  (Hist_Loside_CellV < Vcell_Low_Spec + 0.35)) {
        ...
        // old = config.pinMode(22, INPUT, RISING);     // make 3.7V sense input into a digital interrupt pin, rising edge trigger (1.60V @pin)
        digital.pinMode(22, INPUT, RISING);     // make 3.7V sense input into a digital interrupt pin, rising edge trigger (1.60V @pin)
        What_INT = Snooze.hibernate( config ); // put into hibernate and then return module that woke processor
        for ( ; What_INT != 22; ) {  // loop until input trigger wakes up block, until 3.7V rises (NOTE will hang here if on USB power only)
        What_INT = Snooze.hibernate( config ); // put into hibernate and then return module that woke processor
        }
      }
      else  {   // use Low Power Timer set at 64 seconds to timeout and wakeup processor
        What_INT = Snooze.hibernate( config );// not low cell V so go hibernate for 64 seconds and return module that woke processor
        //Deep sleep/hibernate modes = 700ua with nRF24 and no comms
      }

      DpSleep_timer = TIMEBETWEENSLEEP ;
      ...
      else   {  //input from pin wakeup (22) - low cell charging
        Serial.print("..WAKE UP is from 3.7V interrupt input pin at 3.2V and rising : "); Serial.print(What_INT);
      }
...
// debug new
  ...
  //**************Metrics
        unsigned long debugRXtime = millis();       // get current time
  //**************Metrics
 ...
 SPI.begin();      // re-init all SPI registers after sleep and before every transmission for industrial strength
  ...
  // look for the ACK-SYNC from pack supervisor
      uint8_t len = sizeof(buf);
      uint8_t from;
      uint16_t tempblocknum;
      
       struct dataStruct{
    //float stempValue1;   // want to send floats in the future? //int stempValue2;     // want to send ints in the future
    //unsigned long sSyncRX;  //  // sync time from P.S. to Block
    uint16_t blocknum;  //  
  }ACKSyncpacket;

// new 7/10/2019
  //if (CommTimer < PACKET_DELAY_SEC) goto finito;

   // new 7/11/2019 - change freq for each block - each block gets its own freq channel
   driver.setChannel(CLIENT_ADDRESS);     // set (from default at powerup) channel on each block to be it's own block address
   // new 7/11/2019


  // wait here for ACK-TRIG - special ACK packet from supervisor (non-RF24 official)
      //if (manager.recvfromAckTimeout(buf, &len, 2000, &from))    // this workds!!
      //if (manager.recvfromAckTimeout(buf, &len, 20, &from))    // this workds!!
      // if (manager.recvfromAckTimeout(buf, &len, 400, &from))  // this work - keep less than 1 sec 
      if (manager.recvfromAckTimeout(buf, &len, 800, &from))  // this work - keep less than 1 sec 
    //if (manager.recvfromAckTimeout(buf, &len, 200, &from))  // this work best, too high or too low give problems except at wake up
     {
        if (from == SERVER_ADDRESS)
        {
           tempblocknum = (((dataStruct*)buf) -> blocknum);
           DpSleep_timer = TIMEBETWEENSLEEP * 30;  // restore sleep timer to 120 sec
           Serial.print("got ACK-SYNC packet from P.S.#");
           Serial.println(from, DEC);
           Serial.print("Data requested from Block #  ");Serial.print(tempblocknum ); Serial.println("");
          Serial.print("bytes  RX-TX time = ");  
          Serial.print(millis() - debugRXtime); Serial.print(" msecs");
        ...
        struct DATA {
          float sCellV_Hiside ;     // 4 bytes
          uint16_t sThottest;       // 2 bytes
          uint16_t sTcoldest;
          float sCellV_Loside;
          //
          float schecksum ;
        };
        DATA data;      // 16 byte struct 
     
      
        Serial.print("Preparing block data for supervisor:  ");
      ...
      // now wait here for delay based on BLOCK number to help sequence packets
 //     delay(CLIENT_ADDRESS*11);

      // sometimes the block will miss the client number, or only receive odd or even client numbers so...
      // if client address is this block number - 1, wait n msecs and and then send data...
      // new Jul 9 2019
     // if (tempblocknum == (CLIENT_ADDRESS-1)){
    //    delay(60);
     //   tempblocknum = CLIENT_ADDRESS;
     // }     
      // new Jul 9 2019


          Serial.print("tempblocknum="); Serial.println( tempblocknum);
      
     //if (tempblocknum == CLIENT_ADDRESS)
     if ((tempblocknum == CLIENT_ADDRESS) && (CommTimer >= PACKET_DELAY_SEC))
      {
        CommTimer = 0; // Zero packet timer
        //32 byes maximum RF24 minus 4 bytes of RadioHead headers = 28 bytes for me
        // Send block data to manager_server (Pack Supervisor)and receive short ACK packet back
        if (manager.sendtoWait((uint8_t*)&data, sizeof(data), SERVER_ADDRESS))
        {
            // we are here because server (pack supervisor) sends short ACK packet (not native RF24)
           // continually turn off sleep mode...
           //DpSleep_timer = TIMEBETWEENSLEEP * 30;  // restore sleep timer to 120 sec
           Serial.println("  Sent data to supervisor and got back ACK ");
           Serial.print("bytes  TX-RX-TX-RX time = ");  
           Serial.print(millis() - debugRXtime); Serial.print(" msecs");

           //delay(2); // small delay for TX-RX switch
        }
        else digitalWrite(LED1yellow, GLOW); // turn on yellow led
        commfaults ++;
      }

    }
     }
  else {    // no receive from ACK so no packet
    Serial.println("No ACK-SYNC packet, supervisor not ready?");    // its' possible there was tx but no ACK
    // turn on yellow LED if no ACK packet from server
    digitalWrite(LED1yellow, GLOW);
    Commdelay = 0;
    commfaults ++;
...
finito:
  digitalWrite(LED1yellow, DARK);   // RED led off for low power
  pinMode(LED1green, INPUT);        //  turn off led for low power

  WatchdogReset();  // reset the watchdog timer
  //goto startup;

        Serial.print(commfaults); 
  Serial.println(" = commfaults"); 
